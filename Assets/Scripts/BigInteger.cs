...
namespace BigIntegerType
{
	...
	public class BigInteger
	{
		...
		/// </summary>
		/// <param name="radix">Int radix between 2 and 95</param>
        ...
        #region Other ||| (CODED BY CURTIS MURRAY)
        /// <summary>
        /// Raises a BigInteger to the power of an integer.
        /// </summary>
        /// <param name="leftSide">A BigInteger.</param>
        /// <param name="rightSide">An integer.</param>
        /// <returns>A BigInteger raised to the power of the integer passed in.</returns>
        public static BigInteger operator ^ (BigInteger leftSide, int rightSide)
        {
            if (rightSide == 0)
                return new BigInteger(1);

            BigInteger value = leftSide;
        ...
        /// <param name="leftSide">A BigInteger.</param>
        /// <param name="rightSide">A BigInteger.</param>
        /// <returns>A BigInteger raised to the power of the BigInteger passed in.</returns>
        public static BigInteger operator ^ (BigInteger leftSide, BigInteger rightSide)
        {
            if (rightSide == 0)
                return new BigInteger(1);
        ...
        public string FromString(int radix, string number)
        {
            ...
            for (int i = 0, j = number.Length - 1; j >= 0; i++, j--)
                value += charSet.IndexOf(number[i], 0) * (baseNum ^ j);//(int)Mathf.Pow(95, j);
